...
public class DocumentBenchmarking {
	...
	public static void main(String[] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps * increment + start; numToCheck += increment) {
			 ...
			 * 1. Print out numToCheck
			 * followed by a tab (\t) (NOT a newline) 
			 * 2. Read numToCheck
			 * characters from the file into a String Hint: use the helper
			 * method below. 
			 * 3. Time a loop that runs trials times (trials is
			 * the variable above) that: a. Creates a BasicDocument b. Calls
			 * fleshScore on this document 
			 * 4. Print out the time it took to
			 * complete the loop in step 3 (on the same line as the first print
			 * statement) followed by a tab (\t)
			 * 5. Time a loop that runs trials
			 * times (trials is the variable above) that: a. Creates an
			 * EfficientDocument b. Calls fleshScore on this document 
			 * 6. Print out the time it took to complete the loop in step 5 (on the same
			...
			double timetaken = ((ends - starts) / 1e9f);
			System.out.print(timetaken+"\t");
			
			 starts = System.nanoTime();
			 for(int i = 0;i < trials;i++)  {
				EfficientDocument bd = new EfficientDocument(read);
				bd.getFleschScore();
			}
			 ends = System.nanoTime();
			 timetaken =   ((ends - starts) / 1e9f);
